...
var meConfig = {
    ...
    metags: true,
    isok: false,
    autoReplyMsg: kefuAutoReplyMsg,
    pIndex: 1,//一开始的页数
    pSize: 5,//聊天页面显示信息的条数
    cut: function (tag) {
        this.isVal = tag;
        this.$changeImage.css("display", this.isVal ? "block" : "none");
        this.$sendButton.css("display", !this.isVal ? "block" : "none");
    }
}
...
layui.define(["layer-mobile", "zepto"], function (e) {
     ...
     * 图片压缩
     * @param imgData 图片
     * @param compression 压缩大小
     * @param onCompress 压缩完成回调
     */
    function compressImg(imgData, compression, onCompress) {
        if (!imgData)return;
        ...
        image.onload = function () {
            ...
            canvas.width = w;
            canvas.height = h;
            if (cw > compression && cw > ch) {
                w = compression;
                h = (compression * ch) / cw;
                canvas.width = w;
                canvas.height = h;
            }
            if (ch > compression && ch > cw) {
                h = compression;
                w = (compression * cw) / ch;
                canvas.width = w;
                canvas.height = h;
            ...
            onCompress(canvas.toDataURL("image/jpeg", 0.7));
        }
    }
     ...
     * @returns {*|Window}
     */
    newAlert.msg = function (e) {
        return newAlert.open({
            content: e || "",
            skin: "msg",
            time: 0
        })
    ...
    d.prototype.view = function () {
        ...
        var $editInput = $('.layim-chat-send>input');
        meConfig.$sendButton = $('.layim-send');
        meConfig.$changeImage = $('.change-image');
        ...
        if (!meConfig.isok) {
            ...
            $('#upload_file').change(function () {
                console.log(">>>>>>>>>>>");
                if (meConfig.metags) {
                    meConfig.metags = false;
                    var user_id = localStorage.getItem("user_id");
                    var token = localStorage.getItem("token");
                    var img = event.target.files[0];
                    if (!img) {
                        return;
                        meConfig.metags = true;
                    }
                    ...
                    if (!(img.type.indexOf('image') == 0 && img.type && /\.(?:jpg|png|gif|jpeg)$/.test((img.name).toLowerCase()))) {
                        show_toast("图片只能是jpg、jpeg、png、gif");
                        meConfig.metags = true;
                        return;
                    }
                    ...
                    reader.onload = function (e) {
                        compressImg(e.target.result, 1000, function (data) {
                            $.ajax({
                                type: "post",
                                url: URL.SiteUrl + '/index.php?m=Api&c=User&a=set_chat_image',
                                data: {img: data, user_id: user_id, token: token},
                                async: false,
                                dataType: "json",
                                error: function (data) {
                                    meConfig.metags = true;
                                ...
                                success: function (data) {
                                    if (data.msg != '') {
                                        ...
                                        // show_toast("上传成功");
                                        $('.layim-chat-send>input').val('<img class="image-file" onclick="show_image(event)" src="' + data.result.src + '"/>');
                                        $(".layim-send").click();
...
layui.define(["laytpl", "upload-mobile", "layer-mobile", "zepto"], function (a) {
    ...
    var m = ['<div class="layim-chat layim-chat-{{d.data.type}}{{d.first ? " layui-show" : ""}}">', "{{# if(d.base.chatTitleColor){ }}", '<div id="big_image"></div><div class="layim-chat-title" style="background-color: #67BD49;">', '<p style="position:relative;color:#fff"><a class="return" href="JavaScript:history.go(-1)"></a>{{ d.data.name }}<span class="layim-chat-status"></span></p>', "</div>", "{{# } }}", '<div class="layim-chat-main" {{d.base.chatTitleColor ? "" : "nonetitle"}}>', "<ul></ul>", "</div>", '<div class="layim-chat-footer">', '<div class="layim-chat-send"><div class="layim-chat-tool" data-json="{{encodeURIComponent(JSON.stringify(d.data))}}">', '<span class="layui-icon layim-tool-face" title="选择表情" layim-event="face">&#xe60c;</span></div><input type="text" autocomplete="off"><span class="layui-icon layim-tool-image change-image" title="上传图片" layim-event="image">&#xe60d;<input type="file" name="file" id="upload_file"></span><button class="layim-send" layim-event="send" style="display:none;">发送</button></div>', "{{# if(d.base && d.base.uploadImage){ }}", "{{# }; }}", "{{# if(d.base && d.base.uploadFile){ }}", '<span class="layui-icon layim-tool-image" title="发送文件" layim-event="image" data-type="file">&#xe61d;<input type="file" name="file"></span>', "{{# }; }}", "{{# layui.each(d.base.tool, function(index, item){ }}", '<span class="layui-icon layim-tool-{{item.alias}}" title="{{item.title}}" layim-event="extend" lay-filter="{{ item.alias }}">{{item.icon}}</span>', "{{# }); }}", "</div>", "</div>", "</div>"].join(""),
        r = function (a) {
            return a < 10 ? "0" + (0 | a) : a
        };
    ...
    //  var t='2016-2-10';
    var u, d = ['<li class="layim-chat-li{{ d.mine ? " layim-chat-mine" : "" }}">',
            '<div class="layim-chat-user"><img src="{{ d.avatar }}"><cite>',
            '{{d.mine ? layui.data.date(d.timestamp):""}}    {{ d.username||"佚名" }}    {{d.mine ?"":layui.data.date(d.timestamp)}}',
            "</cite></div>",
            '<div class="layim-chat-text"><img src=""/>{{ layui.data.content(d.content) }}</div>',
            "</li>"].join(""),
        y = {
            message: {},
            chat: []
    ...
    p = function (a) {
        ...
        w = function () {
            ...
            //n.parent().scrollTop(11540);

           // $(".layim-chat-main").animate({scrollTop: 2000}, 100);

            $.ajax({
                type: 'get',
                url: 'http://chat.xionggouba.com:8099/api/chat/getchatrecords',
                data: {userId: meConfig.meown.id, toUserId: meConfig.meto.id, pIndex: 1, pSize: meConfig.pSize},
                dataType: 'json',
                success: function (data) {
                    var head_pic;
                    ...
                    data.push({
                        user_id: 1,
                        nickname: "熊购客服",
                        head_pic: "http://xionggouba.com/Public/upload/head_pic/2017/06-07/5937ca3506e29.png",
                        to_user_id: 0,
                        to_nickname: "",
                        to_head_pic: "",
                        content: meConfig.autoReplyMsg,
                        create_time: String(new Date()),
                        msg_type: 0,
                        is_read: true,
                        msg_id: 0
                    });
                    layui.each(data, function (index, a) {
                        var timestamp2 = Date.parse(a.create_time.replace("T", " "));
                        var clone_a = {
                            content: a.content,
                            id: a.to_user_id,
                        ...
                        if (a.user_id == meConfig.meown.id) {
                            clone_a.mine = true;
                            head_pic = "./images/no-img_mid_.jpg";
                        } else {
                            head_pic = "http://xionggouba.com/Public/upload/head_pic/2017/06-07/5937ca3506e29.png";
                        }
                        clone_a.avatar = a.head_pic ? a.head_pic : head_pic;
                        (new Date).getTime() > a.timestamp && a.timestamp - (g.time || 0) > 6e4 && (n.append('<li class="layim-chat-system"><span>' + layui.data.date(a.timestamp) + "</span></li>"), g.time = a.timestamp), n.append(i(d).render(clone_a))
                    }), C();
                    initMsgBySource();
                }
            })
        ...
        C = function () {
            ...
            if (i.length >= c) {
                var n = i.eq(0);
                t.prev().hasClass("layim-chat-system") || t.before('<p class="up_lodding"><span class="mui-spinner"></span>正在努力加载...</p>');
            }
            /*if ($(uis.up).length > 1) {
             $(uis.up).eq(1).remove();
             }*/
             ...
             */
            refresh({
                content: ".layim-chat-main>ul",
                down: "",
                up: ".up_lodding",
                null: "",
                dif: function () {
                    return ($(".layim-chat-main").scrollTop() == 0);
                }
            }, {
                upCallback: function (e) {
                    ...
                    setTimeout(function () {
                        $.ajax({
                            ...
                            url: 'http://chat.xionggouba.com:8099/api/chat/getchatrecords',
                            data: {
                                userId: meConfig.meown.id,
                                toUserId: meConfig.meto.id,
                                pIndex: meConfig.pIndex,
                                pSize: meConfig.pSize
                            },
                            dataType: 'json',
                            success: function (data) {
                                ...
                                if (!that.isLockUp) {
                                    ...
                                    data = data.reverse();
                                    if (data.length < meConfig.pSize) {
                                        $(".layim-chat-main").find(".up_lodding").remove();
                                        $(".layim-chat-main>ul").before('<div class="layim-chat-system"><span>无更多记录</span></div>');
                                        that.isLockUp = true;
                                    }
                                    var head_pic;
                                    var old = $(".layim-chat-main>ul").height();
                                    layui.each(data, function (index, a) {
                                        var timestamp2 = Date.parse(a.create_time.replace("T", " "));
                                        var clone_a = {
                                            content: a.content,
                                        ...
                                        if (a.user_id == meConfig.meown.id) {
                                            ...
                                            clone_a.username = a.nickname;
                                            head_pic = "./images/def.png";
                                        }
                                        clone_a.avatar = a.head_pic ? a.head_pic : head_pic;
                                        // console.log(clone_a);
                                        $(".layim-chat-main li").eq(0).before(bin(d).render(clone_a));

                                    })
                                    // console.log($(".layim-chat-main>ul").height() - old);
                                    $(".layim-chat-main").scrollTop($(".layim-chat-main>ul").height() - old - 68);
        ...
        k = function () {
            ...
            e.off("keyup").on("keyup", function (a) {
                var i = a.keyCode;
                13 === i && (a.preventDefault(), g()), t["" === e.val() ? "addClass" : "removeClass"]("")
            })
        },
        j = function () {
        ...
        B = {
            ...
            send: function () {
                meConfig.cut(true);
               $(".layim-chat-main").animate({scrollTop: 5000}, 100);
                g()
            },
            face: function (a, e) {
                var i = "",
                ...
                }), t(document).off("click", B.faceHide).on("click", B.faceHide),
                    N(e)
                /*$('.layim-chat-send>input').focus(function(){
                 B.faceHide();
                 })*/
                $('.layui-m-anim-scale').css({"bottom": "10px"});
                $('.layim-chat-footer').css({"bottom": "215px"});
                $('.layim-chat-main').css("bottom", "260px");
            },
            faceHide: function () {
                n.close(n.popBottom.index);
                $('.layim-chat-main').css("bottom", "45px");
                $('.layim-chat-footer').css({"bottom": "0px"});
            },
            image: function (a) {

(function () {
    ...
    //归还商品参数
    var returnGoodinfo = URL.ApiUrl + '&c=Goods&a=lease_msg';
...
//shijianchuo是整数，否则要parseInt转换
    ...
    /*template.helper("cltime", function (data) {
     return format(data);
     });*/
...
// console.log(index);
    ...
    var str = '<div class="noorder noorder_all"><div class="noorder-img"><img src="./images/noorder.png"></div><div class="noorder-title">您还没有相关订单</div></div>';
    ...
    ajax_post({user_id: user_id, token: token}, listUrl, function (data) {
         ...
         */
        //
        // <span class="ex_time"> 总共租借了多久，小时</span>
        // <span class="lease_zuqi"> 免费租期时间，小时</span>


        show_time();
        $('.lease_btn').off('click').on('click', function () {
            var order_id = $(this).attr("data-orderid");
            ajax_post({user_id: user_id, token: token, order_id: order_id}, returnGoodinfo, function (data) {
                if (data.status == 1) {
                    console.log(data);
                    showmask({}, {
                        title: "归还租借商品",
                        content: "<ul class='itemmsg' style='display: inline-block;font-size: 14px; padding-left: 0;'>" +
                        "<li class='itemgold' >租借商品信息</li>" +
                        "<li class='itemzujin'>应付租金￥：" + data.result.zujin + "元</li>" +
                        "<li class='ex_time'>租借时间：" + data.result.ex_time + "小时</li>" +
                        "<li class='lease_zuqi'>免费租期：" + data.result.lease_zuqi + "小时</li>" +
                        "</ul>",
                        contentImage: "",
                        enterButton: "确定",
                        cancelButton: "取消"
        ...
        $('.canNot_btn').off('click').on('click', function () {
            var order_id = $(this).attr("data-orderid");
            var price = $(this).parent().siblings(".indent-list-deta").find(".mp").text();
            // console.log(price);
            plus.webview.open("rentoutPay.html?order_id=" + order_id + "" + "&markprice=" + price + "");
